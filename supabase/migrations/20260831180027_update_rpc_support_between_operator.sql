/*
# Add "between" operator support to search_eb_customers and search_prospects RPCs

1. Changes
- Both `search_eb_customers` and `search_prospects` functions updated to handle the 'between' operator.
- When operator is 'between', the bill condition becomes `column >= value AND column <= max_value`.
- The `max_value` field is read from the bill_conditions JSONB array elements.
- All other operators (gte, gt, lte, lt, eq) continue to work as before.
2. Security
- No RLS or policy changes. Functions remain SECURITY DEFINER with tenant isolation via auth.uid().
*/
  ...
  IF jsonb_array_length(bill_conditions) > 0 THEN
    FOR v_cond IN SELECT jsonb_array_elements(bill_conditions) LOOP
      ...
      IF v_operator = 'between' AND v_max_value IS NOT NULL THEN
        v_bill_clause := v_bill_clause || format('eb.%s >= %s AND eb.%s <= %s',
