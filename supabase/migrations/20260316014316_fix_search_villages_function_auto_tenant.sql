/*
  # Fix search_villages_with_hierarchy to auto-detect tenant_id

  1. Changes
    - Update function to automatically get tenant_id from auth.uid()
    - Make user_tenant_id parameter optional
    - Use the user's tenant_id if not provided
*/

-- Drop and recreate the function with auto-detection
...
'Search villages by name with full location hierarchy. Auto-detects tenant from auth.uid() if not provided.';
