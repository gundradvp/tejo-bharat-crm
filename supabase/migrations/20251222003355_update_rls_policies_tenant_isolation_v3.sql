/*
  # Update RLS Policies for Tenant Isolation (v3)

  ## Overview
  This migration updates all existing RLS policies to include tenant_id filtering for complete
  data isolation between tenants. Uses correct column names from actual schema.

  ## Security Changes
  - All SELECT policies now filter by tenant_id
  - All INSERT policies enforce tenant_id matching
  - All UPDATE policies verify tenant_id ownership
  - All DELETE policies enforce tenant_id matching
...
CREATE POLICY "Users read profiles in tenant"
  ON profiles FOR SELECT
  TO authenticated
  USING (tenant_id = get_user_tenant_id(auth.uid()) OR id = auth.uid());
...
CREATE POLICY "Admins insert profiles in tenant"
...
CREATE POLICY "Admins update profiles in tenant"
  ON profiles FOR UPDATE
  TO authenticated
  USING (
    tenant_id = get_user_tenant_id(auth.uid()) AND
...
CREATE POLICY "Users view customers in tenant"
  ...
  USING (
    ...
    (
      ...
      EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('admin', 'lead_generator'))
    )
  );

CREATE POLICY "Admins insert customers in tenant"
...
CREATE POLICY "Admins delete customers in tenant"
...
-- Tasks policies
CREATE POLICY "Super admins view all tasks"
  ON tasks FOR SELECT
  TO authenticated
  USING (is_super_admin(auth.uid()));

CREATE POLICY "Users view tasks in tenant"
  ON tasks FOR SELECT
  TO authenticated
  USING (
    tenant_id = get_user_tenant_id(auth.uid()) AND
...
CREATE POLICY "Admins insert tasks in tenant"
  ON tasks FOR INSERT
  TO authenticated
  WITH CHECK (
    tenant_id = get_user_tenant_id(auth.uid()) AND
...
CREATE POLICY "Users update tasks in tenant"
  ON tasks FOR UPDATE
  TO authenticated
  USING (
    tenant_id = get_user_tenant_id(auth.uid()) AND
...
CREATE POLICY "Admins delete tasks in tenant"
  ON tasks FOR DELETE
  TO authenticated
  USING (
    tenant_id = get_user_tenant_id(auth.uid()) AND
...
-- Customer notes policies (uses user_id not created_by)
CREATE POLICY "Super admins view all notes"
  ON customer_notes FOR SELECT
  TO authenticated
  USING (is_super_admin(auth.uid()));

CREATE POLICY "Users view notes in tenant"
  ON customer_notes FOR SELECT
  TO authenticated
  USING (
    tenant_id = get_user_tenant_id(auth.uid()) AND
    EXISTS (
      ...
      AND customers.tenant_id = get_user_tenant_id(auth.uid())
    )
  );

CREATE POLICY "Users insert notes in tenant"
...
CREATE POLICY "Users update own notes in tenant"
...
CREATE POLICY "Admins delete notes in tenant"
...
CREATE POLICY "Users view activity in tenant"
  ON activity_logs FOR SELECT
  TO authenticated
  USING (tenant_id = get_user_tenant_id(auth.uid()));

CREATE POLICY "System insert activity in tenant"
...
CREATE POLICY "Users view documents in tenant"
  ON customer_documents FOR SELECT
  TO authenticated
  USING (tenant_id = get_user_tenant_id(auth.uid()));

CREATE POLICY "Users insert documents in tenant"
  ON customer_documents FOR INSERT
  TO authenticated
  WITH CHECK (tenant_id = get_user_tenant_id(auth.uid()));

CREATE POLICY "Users update documents in tenant"
  ON customer_documents FOR UPDATE
  TO authenticated
  USING (tenant_id = get_user_tenant_id(auth.uid()))
  WITH CHECK (tenant_id = get_user_tenant_id(auth.uid()));

CREATE POLICY "Admins delete documents in tenant"
...
CREATE POLICY "Super admins view all statuses"
  ON custom_statuses FOR SELECT
  TO authenticated
  USING (is_super_admin(auth.uid()));

CREATE POLICY "Users view statuses in tenant"
  ON custom_statuses FOR SELECT
  TO authenticated
  USING (tenant_id = get_user_tenant_id(auth.uid()));

CREATE POLICY "Admins manage statuses in tenant"
...
CREATE POLICY "Super admins view all task customers"
  ON task_customers FOR SELECT
  TO authenticated
  USING (is_super_admin(auth.uid()));

CREATE POLICY "Users view task customers in tenant"
  ON task_customers FOR SELECT
  TO authenticated
  USING (tenant_id = get_user_tenant_id(auth.uid()));

CREATE POLICY "Admins manage task customers in tenant"
  ON task_customers FOR ALL
  TO authenticated
  USING (
    tenant_id = get_user_tenant_id(auth.uid()) AND
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('admin', 'lead_generator'))
...
CREATE POLICY "Super admins view all workflow steps"
  ON customer_workflow_steps FOR SELECT
  TO authenticated
  USING (is_super_admin(auth.uid()));

CREATE POLICY "Users view workflow steps in tenant"
  ON customer_workflow_steps FOR SELECT
  TO authenticated
  USING (tenant_id = get_user_tenant_id(auth.uid()));

CREATE POLICY "Users update workflow steps in tenant"
  ON customer_workflow_steps FOR UPDATE
  TO authenticated
  USING (tenant_id = get_user_tenant_id(auth.uid()))
  WITH CHECK (tenant_id = get_user_tenant_id(auth.uid()));