/*
  # Leave Management and Enhanced Attendance System

  ## New Tables

  1. **leave_applications**
     - Stores employee leave requests
     - Fields: leave_type, start_date, end_date, number_of_days, reason, status, supporting_documents
     - Links to profiles for employee and approver
     - Tenant isolation for multi-org support
     - Status workflow: pending → approved/rejected

  2. **daily_task_entries**
     - Stores tasks performed during work day
     - Fields: task_name, task_description, estimated_hours, actual_hours, task_category
     - Linked to attendance_records
     - Multiple tasks per day support

  3. **leave_balances**
     - Tracks leave entitlements and usage per employee
     - Fields: leave_type, total_days, used_days, remaining_days, year
     - Auto-updates on leave approval
     - Annual reset with carry-forward support

  4. **attendance_validation_rules**
     - Configurable attendance policies
     - Fields: late_threshold, grace_period, min_work_hours, geo_fence_locations
     - Tenant-specific configuration

  5. **holidays**
     - Organization holiday calendar
     - Fields: holiday_name, holiday_date, is_optional
...
BEGIN
  IF NEW.status = 'approved' AND OLD.status = 'pending' THEN
    ...
    -- Create absence records in attendance_records for approved leave days
...
BEGIN
  ...
  WHILE v_current_date <= p_end_date LOOP
    ...
    -- Count if it's a weekday (Mon-Fri) and not a holiday
