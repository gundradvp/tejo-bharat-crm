/*
# Update search_eb_customers RPC for solar flag, import batch, and date range filters

1. Changes to search_eb_customers function
- Add p_solar_only (boolean) parameter — when true, filter to solar_already_installed = true
- Add p_import_batch_id (text) parameter — filter by import batch
- Add p_date_from (text) parameter — filter created_at >= this date
- Add p_date_to (text) parameter — filter created_at <= this date

2. Security
- No RLS/policy changes. Function remains SECURITY DEFINER with tenant isolation.
*/
...
CREATE OR REPLACE FUNCTION search_eb_customers(
  ...
  p_solar_only BOOLEAN DEFAULT FALSE,
...
BEGIN
  ...
  IF p_solar_only THEN v_where := v_where || ' AND ec.solar_already_installed = true'; END IF;
