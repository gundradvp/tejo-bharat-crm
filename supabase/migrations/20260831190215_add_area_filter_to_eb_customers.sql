/*
# Add area filter to EB customers

1. get_eb_filter_values: drop and recreate with `areas` array in return
2. search_eb_customers: add `p_area` parameter for comma-separated area filtering
3. Also expand search to include address fields and area_name
4. Security: No RLS/policy changes. Functions remain SECURITY DEFINER with tenant isolation.
*/

DROP FUNCTION IF EXISTS get_eb_filter_values();

CREATE FUNCTION get_eb_filter_values()
RETURNS TABLE(eros TEXT[], sections TEXT[], statuses TEXT[], categories TEXT[], mandals TEXT[], sub_stations TEXT[], areas TEXT[])
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
...
COALESCE(ARRAY_AGG(DISTINCT area_name) FILTER (WHERE area_name IS NOT NULL), ARRAY[]::TEXT[])
FROM eb_customers
WHERE tenant_id = v_tenant_id;
END;
$$;

-- Update search_eb_customers to add p_area parameter and address/area search
