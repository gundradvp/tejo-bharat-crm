...
CREATE POLICY "Super admins view all lookup values" ON lookup_values FOR SELECT TO authenticated USING (is_super_admin(auth.uid()));
CREATE POLICY "Users view lookup values in tenant" ON lookup_values FOR SELECT TO authenticated USING (tenant_id = get_user_tenant_id(auth.uid()));
CREATE POLICY "Admins insert lookup values in tenant" ON lookup_values FOR INSERT TO authenticated WITH CHECK (tenant_id = get_user_tenant_id(auth.uid()) AND EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));
CREATE POLICY "Admins update lookup values in tenant" ON lookup_values FOR UPDATE TO authenticated USING (tenant_id = get_user_tenant_id(auth.uid()) AND EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin')) WITH CHECK (tenant_id = get_user_tenant_id(auth.uid()) AND EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));
CREATE POLICY "Admins delete non-system lookup values in tenant" ON lookup_values FOR DELETE TO authenticated USING (tenant_id = get_user_tenant_id(auth.uid()) AND EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin') AND is_system = false);

DROP TRIGGER IF EXISTS update_lookup_values_updated_at ON lookup_values;
