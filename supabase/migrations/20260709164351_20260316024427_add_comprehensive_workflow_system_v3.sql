-- Drop existing loan_disbursements table (old schema, no data)
...
-- Create loan_disbursements table (new schema with loan_application_id)
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'customers' AND column_name = 'current_workflow_stage') THEN
    ALTER TABLE customers ADD COLUMN current_workflow_stage text DEFAULT 'site_survey';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'customers' AND column_name = 'workflow_stage_updated_at') THEN
...
-- Insert default workflow stages for all tenants
